...
class internlm2Chat(BaseLLM):
    ...
    def get_pred(self, data, max_length, max_gen, prompt_format, device, out_path):
        ...
        for json_obj in tqdm(data):
            ...
            context_length = input.input_ids.shape[-1]  # 喂进去的tokens的长度
            ...
            with open(out_path, "a", encoding="utf-8") as f:
                json.dump(
                    {"pred": pred, "answers": json_obj["output"], "all_classes": json_obj.get("all_classes", None),
                     "length": json_obj.get("length", None)}, f, ensure_ascii=False)
                f.write('\n')
